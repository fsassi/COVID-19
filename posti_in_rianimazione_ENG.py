...
plt.figure()
ax = plt.axes()
plt.title('ITALIAN PATIENTS in ICU\n' + \
          'Last data point: ' + str(data_naz['data'].iloc[-1]) + '\n' + \
          "Interpolation to " + str(FUTURE_DAYS) + ' days')
...
labels.append('Quadratic interpolation')

handles.append(Line2D([0], [0], color='k', linewidth=3, linestyle='--'))
labels.append('Exponential interpolation')

handles.append(Line2D([0], [0], color='g', linewidth=3, linestyle='-'))
labels.append('Currently in ICU')

handles.append(Line2D([0], [0], color='orange', linewidth=3, linestyle='-'))
labels.append('Twice the current in ICU')

handles.append(Line2D([0], [0], color='r', linewidth=3, linestyle='-'))
labels.append('Total available ICU beds in Italy')
